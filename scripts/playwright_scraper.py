                    ...
                    while no_new_rounds < 5:
                        ...
                        if current_visible_hashes == previous_visible_hashes:
                            ...
                            if current_db_count > previous_db_count:
                                print(
                                    f"Batch Scraped! Added {current_db_count - previous_db_count} new comments. (Total Unique: {current_db_count})"
                                )
                            else:
                                print(
                                    f"Batch Scraped! Bypassing already saved comments... (Visible: {len(current_visible_hashes)})"
                                )
        ...
        # --- FINALLY BLOCK ALWAYS RUNS, EVEN ON CRASH OR CTRL+C ---
            ...
            if collected:
                ...
                with open(FILE_PATH, "w", encoding="utf-8") as f:
                    json.dump(comments, f, ensure_ascii=False, indent=2)
                print(
                    f"✅ Emergency Save Complete! {len(comments)} comments secured in {FILE_PATH}."
                )
...
def main_v2():
    ...
    while crash_attempts < MAX_CRASH_RETRIES:
            ...
            with Stealth().use_sync(sync_playwright()) as p:
                ...
                while True:
                    ...
                    if consecutive_refreshes < 2:
                        ...
                        print(
                            f"🔄 Auto-Recovery Attempt {consecutive_refreshes}/2: Trying to clear UI and refresh..."
                        )
                        ...
                        try:
                            ...
                            print(
                                "Page reloaded successfully. Fast-forwarding through saved comments..."
                            )
                        except Exception as e:
                        ...
                        print(
                            "Please look at the browser window to solve the block (CAPTCHA, Login popup, etc.)."
                        )
        ...
        # Catch the "Target Closed" / "Page Crashed" Exception
            ...
            print(
                f"⚠️ Recovering... Crash {crash_attempts}/{MAX_CRASH_RETRIES}. Restarting to clear RAM..."
            )
